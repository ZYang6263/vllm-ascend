...
def _build_plan(
    ...
    if topk_size <= 0 or num_routed_experts <= 0 or moe_layer_frequency <= 0:
        return SfaPrefetchPlan(topk_size, 0, tuple(roles))

    def is_moe_layer(layer_id: int) -> bool:
        return layer_id >= first_dense_layers and (layer_id - first_dense_layers) % moe_layer_frequency == 0
    ...
    for full_layer_id, layer_type in enumerate(layer_types):
        ...
        if producer_layer_id is None:
            continue
        target_layer_ids = tuple(layer_id for layer_id in shared_layer_ids if layer_id > producer_layer_id)
        if not target_layer_ids:
            continue
        roles[producer_layer_id] = SfaPrefetchRole(target_layer_ids=target_layer_ids)
