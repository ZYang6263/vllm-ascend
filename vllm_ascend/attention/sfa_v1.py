...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.has_indexer:
            ...
            self.n_head: int = getattr(hf_config, "index_n_heads", 0)
            self.head_dim: int = getattr(hf_config, "index_head_dim", 0)
            self.wq_b = None
    ...
    def indexer_select_pre_process(
        ...
        if not self.has_indexer:
            raise RuntimeError(
                "indexer_select_pre_process should not be called when "
                f"indexer is None. layer_name={self.layer_name}."
            )
    ...
    def indexer_select_post_process(
        ...
        if not self.has_indexer:
            raise RuntimeError(
                "indexer_select_post_process should not be called when "
                f"indexer is None. layer_name={self.layer_name}."
            )
    ...
    def forward(
        ...
        if self.enable_dsa_cp:
            topk_num_tokens = ql_nope.shape[0]
        else:
