...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def _ensure_prefetch_buffer(self, kv_cache) -> None:
        ...
        if (
            ...
            and AscendSFAImpl._prefetch_current_topk_slots.shape == (max_tokens, 8)
    ...
    def _ensure_prefetch_target_kv_caches(self) -> None:
        """Resolve the producer follower caches from the static context."""
        if not self.is_prefetch_producer or self.prefetch_target_kv_caches is not None:
            return
        context = self.vllm_config.compilation_config.static_forward_context
        caches = []
        for layer_id in self.prefetch_target_layer_ids:
            layer_context = context.get(get_layer_name_with_index(self.layer_name, layer_id))
        ...
        self.prefetch_target_kv_caches = caches

    def _get_prefetch_slice(self, num_actual_tokens: int):
        if self.prefetch_buffer_index < 0 or AscendSFAImpl._prefetched_kv is None:
            return None, None
    ...
    def _patch_prefetched_current_tokens(
        ...
        if current_topk_slots is None:
            raise RuntimeError("Grouped SFA prefetch did not publish current-token slots.")
    ...
    def forward(
        ...
        if self.skip_topk:
            ...
            if self.prefetch_enabled and self.prefetch_buffer_index >= 0 and AscendSFAImpl._prefetched_kv is not None:
                ...
                if prefetched_kv is not None and attn_metadata.attn_state == AscendAttentionState.DecodeOnly:
                    ...
                    valid_kv_lengths = [
                        min(seq_len, kv_len) for seq_len in attn_metadata.seq_lens_cpu[:num_actual].tolist()
                    ]
        ...
        if (
            self.prefetch_enabled
            and self.is_prefetch_producer
            and attn_metadata.attn_state == AscendAttentionState.DecodeOnly
        ):
            self._ensure_prefetch_buffer(kv_cache)
            self._ensure_prefetch_target_kv_caches()
            if self.prefetch_target_kv_caches is not None:
                num_actual_tokens = attn_metadata.num_actual_tokens
                topk_indices_for_prefetch = topk_indices[:num_actual_tokens].reshape(num_actual_tokens, -1)
                if attn_metadata.prefetch_positions is None:
                    raise RuntimeError("SFA prefetch requires INT32 positions metadata.")
                current_positions = attn_metadata.prefetch_positions[:num_actual_tokens]
                current_topk_slots = AscendSFAImpl._prefetch_current_topk_slots
                assert current_topk_slots is not None
                current_topk_slots = current_topk_slots[:num_actual_tokens]
                attn_metadata.prefetch_current_topk_slots = current_topk_slots
                ...
                with torch_npu.npu.stream(gather_stream):
                    ...
                    prefetch_outputs = [
                        (
                            AscendSFAImpl._prefetched_kv[index][:num_actual_tokens],
                            AscendSFAImpl._prefetched_k_pe[index][:num_actual_tokens],
                        )
                        for index in range(3)
                    ]
                    if num_prefetch_layers > len(prefetch_outputs):
                        raise ValueError(
                            "SFA prefetch group has more target layers than the grouped gather operator input slots."
                        )
                    ctkv_0, kpe_0 = target_kv_caches[0]
                    ctkv_1, kpe_1 = target_kv_caches[1] if num_prefetch_layers > 1 else target_kv_caches[0]
                    ctkv_2, kpe_2 = target_kv_caches[2] if num_prefetch_layers > 2 else target_kv_caches[0]
