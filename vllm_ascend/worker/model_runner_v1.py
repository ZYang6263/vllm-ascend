...
class NPUModelRunner(GPUModelRunner):
    ...
    @staticmethod
    def _sparse_attention_has_indexer(attn_module: MLAAttention) -> bool:
        # The attention implementation owns the runtime Indexer path, so prefer
        # its resolved state over the wrapper attribute. The latter is retained
        # as a compatibility fallback for backends without ``has_indexer``.
        impl = getattr(attn_module, "impl", None)
        impl_has_indexer = getattr(impl, "has_indexer", None)
        if impl_has_indexer is not None:
            return bool(impl_has_indexer)
        return attn_module.indexer is not None
    ...
    def get_kv_cache_spec(self) -> dict[str, KVCacheSpec]:
        ...
        for layer_name, attn_module in attn_layers.items():
            if (isinstance(attn_module, Attention)
                ...
                if self.use_sparse:
                    has_indexer = self._sparse_attention_has_indexer(attn_module)
                    if has_indexer:
